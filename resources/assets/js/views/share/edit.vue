    ...
    <div id="content">
        ...
        <div id="form">
            <el-form :model="form" :rules="rules" ref="form" label-width="100px" class="demo-ruleForm">
                ...
                <el-form-item>
                    <el-button type="primary" @click="submitForm('form')">立即更新</el-button>
                </el-form-item>
            </el-form>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        window.axios.put('/api/shares/' + this.$route.params.id , formData)
                            .then(response => {
                                loading.close();
                                this.$notify({
                                    title: '成功',
            ...
            fatchData(){
                window.axios.get('/api/shares/' + this.$route.params.id ,{
                    params:{
                        include:'tags'
                    }
                }).then(response => {
                    let share = response.data.data;
                    this.form.title = share.title;
                    this.form.tags = share.tags;
                    this.form.fileList = [{
                        name:share.logo,
                        url : share.logo
                    }];
                    this.form.logo = share.logo;
                    this.form.content = share.content;
                    this.form.link = share.link;
                })
            },
        }
    }
